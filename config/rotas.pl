...
:- http_handler( root(propostas), entradaProposta,   []).
...
:- http_handler(root(proposta), cadastroProposta , []).
...
:- http_handler(root(proposta/editar/Id), editarProposta(Id) , []).
...
:- http_handler( api1(api_proposta/Id),api_proposta(Metodo, Id),
                 [ method(Metodo),
                   methods([ get, post, put, delete ]) ]).
