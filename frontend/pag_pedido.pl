...
entradaPedido(_):-
    reply_html_page(
        boot5rest,
        [ title('ObservaÃ§Ãµes da Proposta')],
        [ div(class(container),
              [ \html_requires(css('entrada.css')),
                \html_requires(js('bookmark.js')),
                \titulo_da_paginaped('Meus Pedidos'),
                \tabela_de_bookmarkspedido

              ]) ]).

titulo_da_paginaped(Titulo) -->
    html( div(class('text-center align-items-center w-100'),
              h1('display-3', Titulo))).

tabela_de_bookmarkspedido -->
    html(div(class('container-fluid py-3'),
             [
               \cabeca_da_tabelaped('     ', '/','voltar'),
               \cabeca_da_tabelaped('Pedidos',
               '/pedido','Novo Pedido'), \tabelapedido ] )).


cabeca_da_tabelaped(Titulo,Link,Nome) -->
...
tabelapedido -->
    html(div(class('table-responsive-md'),
             table(class('table table-striped w-100'),
                   [ \cabecalhopedido,
                     tbody(\corpo_tabelapedido)
                   ]))).

cabecalhopedido -->
    html(thead(tr([ th([scope(col)], '#'),
                    th([scope(col)], 'Cod. Observação'),
                    th([scope(col)], 'Observação'),
					th([scope(col)], 'Data Observação'),
...
corpo_tabelapedido -->
    {
        findall( tr([th(scope(row), Id), td(CodObserv), td(Observ), td(DataObserv),td(Usuario),td(Acoes)]),
                 linhapedido(Id, CodObserv, Observ, DataObserv,Usuario, Acoes),
                 Linhas )
    },
    html(Linhas).
...
linhapedido(Ido,CodObserv, Observ, DataObserv,Usuario, Acoes):-
    pedido:pedido(Ido,CodObserv, Observ, DataObserv,Usuario),
    acoespedido(Ido,Acoes).


acoespedido(Id, Campo):-
    Campo = [ a([ class('text-success'), title('Alterar'),
                  href('/pedido/editar/~w' - Id),
                  'data-toggle'(tooltip)],
                [ \lapis ]),
              a([ class('text-danger ms-1'), title('Excluir'),
                  href('/api/v1/api_pedido/~w' - Id),
                  onClick("apagar( event, '/pedidos' )"),
                  'data-toggle'(tooltip)],
                [ \lixeira ])
            ].


% ÃƒÂcones do Bootstrap 5
/*
lapis -->
